...
using namespace alloc_test::threading::impl;
using namespace alloc_test::threading;
using namespace alloc_test;

priority_task notification_queue::try_pop(int priority) {
    assert(priority < (int)q_tasks_.size());
...
bool notification_queue::try_push(priority_task& ptsk) {
    assert(ptsk.priority < (int)q_tasks_.size());
    {
        lock q_lock{q_mutex_, std::try_to_lock};
...
void task_system::run(priority_task ptsk) {
    assert(ptsk);
    auto guard = on_scope_exit([pri = current_task_priority_] { current_task_priority_ = pri; });
...
void task_system::run_tasks_loop(int index) {
    auto guard = on_scope_exit([] { current_task_queue_ = -1; });
    current_task_queue_ = index;
    if (bind_) set_affinity(index, count_);
    while (true) {
        priority_task ptsk;
...
task_system::task_system(int nthreads, bool bind):
    ...
    if (bind_) set_affinity(0, count_);
...
void task_system::async(priority_task ptsk) {
    if (ptsk.priority>=n_priority) {
        ...
        assert(ptsk.priority < (int)index_.size());
